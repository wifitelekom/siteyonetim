@extends('layouts.app')
@section('title', 'Yeni Site')

@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <h1 class="sy-page-title">Yeni Site</h1>
    ...
    <form method="POST" action="{{ route('super.sites.store') }}" class="space-y-4">
        @csrf
        ...
        <section class="sy-card p-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input class="sy-input" type="text" id="name" name="name" value="{{ old('name') }}" required>
                </div>
                <div>
                    <label class="sy-label" for="phone">Telefon</label>
                    <input class="sy-input" type="text" id="phone" name="phone" value="{{ old('phone') }}">
                </div>
                <div>
                    <label class="sy-label" for="tax_no">Vergi No</label>
                    <input class="sy-input" type="text" id="tax_no" name="tax_no" value="{{ old('tax_no') }}">
                </div>
                <div class="md:col-span-2">
                    <label class="sy-label" for="address">Adres</label>
                    <textarea class="sy-input min-h-[90px]" id="address" name="address">{{ old('address') }}</textarea>
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="is_active" name="is_active" value="1" class="h-4 w-4" {{ old('is_active', '1') ? 'checked' : '' }}>
        ...
        <section class="sy-card p-6">
            ...
            <div class="mb-4">
                <label class="sy-label" for="admin_user_id">Mevcut Yonetici Sec (opsiyonel)</label>
                <select id="admin_user_id" name="admin_user_id" class="sy-input">
                    <option value="">Yeni yonetici olustur</option>
                    @foreach($availableAdmins as $admin)
                        <option value="{{ $admin->id }}" @selected(old('admin_user_id') == $admin->id)>
                            {{ $admin->name }} - {{ $admin->email }}
                        </option>
                    @endforeach
                </select>
                <p class="mt-2 text-xs text-slate-500">Secerseniz asagidaki yeni yonetici alanlari yok sayilir.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="sy-label" for="admin_name">Yonetici Ad Soyad *</label>
                    <input class="sy-input" type="text" id="admin_name" name="admin_name" value="{{ old('admin_name') }}">
                </div>
                <div>
                    <label class="sy-label" for="admin_email">Yonetici E-posta *</label>
                    <input class="sy-input" type="email" id="admin_email" name="admin_email" value="{{ old('admin_email') }}">
                </div>
                <div>
                    <label class="sy-label" for="admin_password">Yonetici Sifre *</label>
