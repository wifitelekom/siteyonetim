...
const fetchDetail = async () => {
  ...
  try {
    const response = await withAbort(signal => $api<ApartmentDetailResponse>(`/apartments/${apartmentId.value}`, { signal }))
    form.value = {
      block: response.data.block ?? '',
      floor: response.data.floor,
      number: response.data.number,
      m2: response.data.m2,
      arsa_payi: response.data.arsa_payi,
...
const deleteApartment = async () => {
  ...
  try {
    await $api(`/apartments/${apartmentId.value}`, { method: 'DELETE' })
