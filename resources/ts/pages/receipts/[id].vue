...
interface ReceiptShowResponse {
  data: ReceiptDetail
}

const route = useRoute()
const receiptId = computed(() => Number((route.params as Record<string, unknown>).id))
...
const fetchDetail = async () => {
  ...
  try {
    const response = await $api<ReceiptShowResponse>(`/receipts/${receiptId.value}`)
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard :loading="loading">
        <VCardItem title="Tahsilat Kalemleri" />
        ...
        <VTable density="comfortable">
          ...
          <tbody>
            <tr
              ...
              :key="item.id"
            >
              <td>{{ item.charge?.account?.name ?? '-' }}</td>
